...
class Watermarker(object):
    def __call__(self, url, name, position=None, opacity=0.5, tile=False,
        ...
        * ``quality``: the quality of the resulting watermarked image.  Default
        ...
        target_path = smart_str(self.get_url_path(url))
    ...
    @staticmethod
    def get_url_path(url, root=settings.MEDIA_ROOT,
    ...
    @staticmethod
    def watermark_name(mark, **kwargs):
    ...
    def watermark_path(self, basedir, base, ext, wm_name, obscure=True):
        ...
        watermark_hash = sha1(smart_str(wm_name)).hexdigest()

        # figure out where the watermark would be saved on the filesystem
        if obscure:
            log.debug('Obscuring original image name: %s => %s' % (wm_name, watermark_hash))
            new_file = os.path.join(basedir, watermark_hash + ext)
        else:
            log.debug('Not obscuring original image name.')
            new_file = os.path.join(basedir, watermark_hash, base + ext)
    ...
    @staticmethod
    def create_watermark(target, mark, path, quality=QUALITY, **kwargs):
        """Create the watermarked image on the filesystem"""

        im = utils.watermark(target, mark, **kwargs)
        #        if path.split('.')[-1].upper() == 'BMP':
        if len(im.split()) == 4:
            r, g, b, a = im.split()
